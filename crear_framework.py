    ...
    if modo == "STRESS":
        ...
        with user.client.post(base_path, json=payload, catch_response=True, name="Petición_Prueba_Estrés") as response:
                    ...
                    # Calculamos cuánto tardó realmente la petición
                        ...
                        if tiempo_real > TIEMPO_MAXIMO_ACEPTABLE:
                            response.failure(f"SLA ROTO: tardó {tiempo_real:.2f}s (Límite: {TIEMPO_MAXIMO_ACEPTABLE}s)")
                        else:
        ...
        with user.client.get(base_path, catch_response=True, name="Prueba_Carga") as response:
            ...
            if tiempo_segundos > TIEMPO_MAXIMO_ACEPTABLE:
                mensaje_error = f"Se presentó lentitud de {response.elapsed.total_seconds()} ms"
                print(f"🟠🟠 {mensaje_error}")
                response.failure(f"Lentitud: {response.elapsed.total_seconds():.2f}s")
...
def analizar_resultados(archivo_base, tipo_prueba, total_users, umbral_tiempo_ms=2000):
    """
...
def ejecutar_prueba():
    ...
    try:
        ...
        # 6. Análisis final
        analizar_resultados(ruta_archivo_base, tipo, users)
